...
int lScore, rScore;
int lastLY, beforeLY = 0, lastRY, beforeRY = 0;
...
void dealData(int digital){
    if(digital == 1  &&  lPaddle.position.y != 80) {//左
      lPaddle.position.y -= 10;
    }
    if(digital == 2  &&  lPaddle.position.y != 640) {//右
        lPaddle.position.y += 10;
    }
    if(digital == 3  &&  rPaddle.position.y != 80) {//下
        rPaddle.position.y -= 10;
    }
    if(digital == 4  &&  rPaddle.position.y != 640) {//上
        rPaddle.position.y += 10;
    }
}
