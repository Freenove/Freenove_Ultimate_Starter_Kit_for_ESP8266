...
void loop() {
  ...
  for (int j = 0; j < 500; j++ ) {  // repeat 500 times
    ...
    for (int i = 0; i < 8; i++) {   // display 8 column data by scanning
      matrixColsVal(smilingFace[i]);// display the data in this column
      matrixRowsVal(~cols);          // select this column
      delay(1);                     // display them for a period of time
      matrixColsVal(0x00);          // clear the data of this column
  ...
  for (int i = 0; i < 128; i++) { 
    for (int k = 0; k < 10; k++) {      // repeat image of each frame 10 times.
      ...
      for (int j = i; j < 8 + i; j++) { // display image of each frame
        matrixColsVal(pgm_read_word_near(data + j));// display the data in this column
        matrixRowsVal(~cols);            // select this column
        delay(1);                       // display them for a period of time
        matrixColsVal(0x00);            // close the data of this column
        cols <<= 1;                 // shift "cols" 1 bit left to select the next column
      }
    }
  }
